...
//    }
    ...
    public void init() {
        ...
        ttc_id.setPrefWidth(250);
        ttc_amount.setPrefWidth(250);
        ttc_date.setPrefWidth(250);
        ttc_payer.setPrefWidth(250);

        tf_filter.textProperty().addListener((observable, oldValue, newValue) -> {
            this.ttv.setPredicate((TreeItem<Transaction> t) -> {
                Boolean flag = t.getValue().date.getValue().contains(newValue)
                        || t.getValue().id.getValue().toString().contains(newValue)
                        || t.getValue().payer_Wallet.getValue().toString().contains(newValue)
                        || t.getValue().amount.getValue().toString().contains(newValue);
                return flag;
            });
        });
...
//        });
        ...
        ttv.getColumns().setAll(ttc_id, ttc_amount, ttc_payer, ttc_date);
        ttv.setRoot(root);
